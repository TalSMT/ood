...
public class SamplerLocation {
 ...
 public static ArrayList<SampleOfWifi> picalc (ArrayList<SampleOfWifi> allWifiSmp, ArrayList<macSamlpe> input)
 {
  ...
  int sumifthereistheInputInLine=0;
  for (int i = 0; i<allWifiSmp.size(); i++) { //Move through all the lines in the file

   ArrayList<WifiPoint> currentLine = allWifiSmp.get(i).getwifiSpotList();
   for (int j = 0; j < input.size(); j++) {//Move through all the inputs
    for (int j2 = 0; j2 < currentLine.size(); j2++) {//We go through the whole line
     if( currentLine.get(j2).getMac().equals(input.get(j).getMac()))//Check whether this line contains one of the MAC addresses from the input
     {
      yeshbashura= true;
      sumifthereistheInputInLine++;
      w= weightcalc(currentLine.get(j2), input.get(j));//Calculate the weighted average
      pi=pi*w;
      break;
     }
    ...
    {
     // If the MAC address was not found in the row, The weighted average is calculated according to a constant variable 

     mehane=((Math.pow(Constant.diffNoSig, Constant.sigDiff))*(Math.pow(input.get(j).getSignal(), Constant.power)));
     mehane= (int)mehane;
     pi=(Constant.norm/mehane);
  ...
  if(sumifthereistheInputInLine==0){
   //pi=0;
  									}
   allWifiSmp.get(i).setPi(pi);
   pi=1;
   sumifthereistheInputInLine=0;
  }
  return allWifiSmp;
 }
 ...
 public static double weightcalc (WifiPoint smp, macSamlpe input ){
  ...
  double w=Constant.norm/((Math.pow(diffcalc(smp, input), Constant.sigDiff))*(Math.pow(input.getSignal(), Constant.power)));
